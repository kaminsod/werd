...
export default function PostDetailPage() {
  ...
  function handleSave(e: FormEvent) {
    e.preventDefault();
    updatePost.mutate(
      { postId: postId!, title, content, url: postURL, post_type: postType, platforms: selectedPlatforms },
              ...
              {availablePlatforms.map((p) => {
                ...
                return (
                  <label key={p} className="flex items-center gap-1.5">
                    <input type="checkbox" checked={selectedPlatforms.includes(p)} onChange={() => setSelectedPlatforms((prev) => prev.includes(p) ? prev.filter((x) => x !== p) : [...prev, p])} />
        ...
        <div className="space-y-4 rounded border bg-white p-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="mt-1">{post.post_type}</p>
            </div>
            <div>
